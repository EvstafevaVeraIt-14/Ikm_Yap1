...
int main(){
    ...
    cout << "------------------------------------------------------------------------------------------------------------------------" << endl;
    while (!fl) { //запрос ввода до допустимых исходных данных
        cout << "Введите число N (N>40): ";
        getline(cin, str);
        if (proverka(str)) {  //проверка на корректность ввода
            n = izstrvint(str); //переформатирование исходных данных

            if (n > 40) { //проверка на условие задачи
                umenshalka(n, a, size); //вычисления
                cout << "2^" << n << "-1 = ";
                vyvod(a, size); //вывод результата
                fl = true; 
            }
            else {
